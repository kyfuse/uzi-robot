...
def _worker(on_utterance: Callable[[str, bool], None]) -> None:
    ...
    while not _stop.is_set():
        ...
        try:
            ...
            is_endpoint = False
        if is_endpoint:
            if _muted.is_set() or text.strip() == "Okay." or text.strip() == "Okay?":  # Weird case when there's silence
...
if __name__ == "__main__":
    start(on_utterance=lambda text, is_final: print(f"{text!r} {'FINAL' if is_final else 'PARTIAL'}"))
